...
- **Tables**: `@tiptap/extension-table` (Table, TableRow, TableHeader, TableCell). Inserted via `/table` slash command (3×3 with header row). Tab/Shift+Tab navigates cells; Tab on the last cell appends a new row. Alt+Shift+Arrow shortcuts add rows/columns. `src/components/TableMenu/TableMenu.jsx` is a `BubbleMenu`-based floating toolbar (shown when `editor.isActive('table')`) for row/column insert and delete operations.
